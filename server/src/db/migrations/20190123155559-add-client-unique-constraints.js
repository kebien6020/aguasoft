...
module.exports = {
  up: (queryInterface, Sequelize) => {
    ...
    return sequelize.query('PRAGMA foreign_keys = OFF;', raw).then(() =>
      ...
      queryInterface.addConstraint('Clients', ['name'], nameOptions)
    ).then(() =>
      queryInterface.addConstraint('Clients', ['code'], codeOptions)
    ).then(() =>
  ...
  down: (queryInterface, Sequelize) => {
    ...
    return sequelize.query('PRAGMA foreign_keys = OFF;', raw).then(() =>
      ...
      queryInterface.removeConstraint('Clients', NAME_UNIQUE, addCommon())
    ).then(() =>
      queryInterface.removeConstraint('Clients', CODE_UNIQUE, addCommon())
    ).then(() =>
      sequelize.query('COMMIT;', raw)
    ).catch(() =>
      sequelize.query('ROLLBACK;', raw)
